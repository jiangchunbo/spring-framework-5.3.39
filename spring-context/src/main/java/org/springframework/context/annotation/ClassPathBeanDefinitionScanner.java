...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	public ClassPathBeanDefinitionScanner(BeanDefinitionRegistry registry) {
		// 使用默认的过滤器 -> 加入一些 include filters，默认将 @Component 等注解扫描进来
		this(registry, true);
	}
	...
	public int scan(String... basePackages) {
		// 获取当前 registry 中有多少个 bean definition，待会统计扫描了多少个
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			// 读取 class 文件，过滤，构造 bean definition
			// >>>> 得到的 BeanDefinition 没有填充属性
			Set<BeanDefinition> candidates = findCandidateComponents(basePackage);

			// 填充 BeanDefinition 属性
			for (BeanDefinition candidate : candidates) {
				ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(candidate);
				candidate.setScope(scopeMetadata.getScopeName());
				String beanName = this.beanNameGenerator.generateBeanName(candidate, this.registry);

				// 给 Bean Definition 填充默认值
	 ...
	 * <p>
	 * 对指定的 bean 定义应用额外配置，补充扫描组件类时获取的内容。
	 *
	...
	protected boolean checkCandidate(String beanName, BeanDefinition beanDefinition) throws IllegalStateException {
		...
		// 检查兼容性，如果兼容，就没必要再处理一次了，return false；否则，抛出异常
	 ...
	 * <p>
	 * 给定的新的 bean definition 是否兼容已经存在的 bean definition
	 *
	...
	protected boolean isCompatible(BeanDefinition newDef, BeanDefinition existingDef) {
		...
		return
				// 当前 bd 不是扫描得到的，这里隐含了非扫描则“有意”，有意 >= 无意，有意覆盖无意
				(!(existingDef instanceof ScannedGenericBeanDefinition) ||  // explicitly registered overriding bean
						// 底层的资源都是同一个，扫描了相同的文件两次
