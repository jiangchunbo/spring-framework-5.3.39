...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void publishEvent(Object event, @Nullable ResolvableType eventType) {
		...
		if (event instanceof ApplicationEvent) {
			...
			// 你发布的是一个 payload 对象，将会使用 PayloadApplicationEvent 将 event 承载起来
		...
		// 如果是早期，那么只需要添加到 earlyApplicationEvents 即可，暂时不发布
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 注册 4 个 registerResolvableDependency
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// Web 环境下注册 4 个 registerResolvableDependency
				...
				// 注册 BeanPostProcessor，同时会这些组件也都会创建出来
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		beanFactory.registerResolvableDependency(BeanFactory.class, beanFactory); // 手工注册 BeanFactory
		beanFactory.registerResolvableDependency(ResourceLoader.class, this); // 手工注册 ResourceLoader
		beanFactory.registerResolvableDependency(ApplicationEventPublisher.class, this); // 手工注册 ApplicationEventPublisher
		beanFactory.registerResolvableDependency(ApplicationContext.class, this); // 手工注册 ApplicationContext
	 ...
	 * <p>
	 * 初始化事件多播器，如果容器中没有自己定义，那么使用 SimpleApplicationEventMulticaster
	 *
	...
	protected void registerListeners() {
		...
		// 首先，注册静态指定的监听器
		// 就是通过 ApplicationContext  addApplicationListener 方法添加的监听器
		...
		// 从容器中获得类型是 —— ApplicationListener 的 bean
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 注入 Conversion Service，其实就是从单例池中找到这个 Bean，然后设置到一个专门的属性中
		if (beanFactory.containsBean(CONVERSION_SERVICE_BEAN_NAME) &&
				beanFactory.isTypeMatch(CONVERSION_SERVICE_BEAN_NAME, ConversionService.class)) {
			// Spring Boot 没有走进这个分支，不清楚为什么
