 ...
 * <p>
 * 封装了 Java Type 对象，提供易于操作和导航的接口，以实现对类型、泛型参数的解析及其父类、接口的访问。
 *
...
public class ResolvableType implements Serializable {
	...
	private boolean isUnresolvableTypeVariable() {
		// 如果类型是 TypeVariable，也就是譬如 T 这种，说明不可解析类型
		if (this.type instanceof TypeVariable) {
			// 如果解析器也没有，那就真的没法解析了
			if (this.variableResolver == null) {
				return true;
	...
	private Class<?> resolveClass() {
		...
		// 泛型数组
		if (this.type instanceof GenericArrayType) {
			// 获得数组 ComponentType 的 ResolvableType
	...
	ResolvableType resolveType() {
		// 类型是 ParameterizedType，带有泛型类型实参的类型 List<E> List<?> 等
		if (this.type instanceof ParameterizedType) {
			// 两个参数，第一个参数是原始类型，例如 List
			return forType(((ParameterizedType) this.type).getRawType(), this.variableResolver);
		}

		// 类型是 WildcardType，也就是可能是一些问号 ?
		if (this.type instanceof WildcardType) {
			// 获取上边界 ? extends A   此时 A 就是上边界
			// 获取下边界 ? super B     此时 B 就是下边界
