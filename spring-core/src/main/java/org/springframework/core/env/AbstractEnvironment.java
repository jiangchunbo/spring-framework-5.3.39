...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	/**
	 * 一个 {@code Iterable<PropertySource>} 对象。你也可以认为就是一个 List。而且他底层也就是 CopyOnWriteArrayList
	 */
	...
	public AbstractEnvironment() {
		// 无参构造器，默认传入一个空的 MutablePropertySources
	...
	protected AbstractEnvironment(MutablePropertySources propertySources) {
		// 这个构造器专门为 MutablePropertySources 打造，参数类型强制是 MutablePropertySources

		this.propertySources = propertySources;
		this.propertyResolver = createPropertyResolver(propertySources);

		// 定制化 MutablePropertySources
		// 一开始传入的 MutablePropertySources 一般不就是空的么，定制化就是加一些属性，比如加系统属性、环境变量
