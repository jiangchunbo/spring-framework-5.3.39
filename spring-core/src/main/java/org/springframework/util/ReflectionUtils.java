...
public abstract class ReflectionUtils {
	...
	public static void doWithMethods(Class<?> clazz, MethodCallback mc, @Nullable MethodFilter mf) {
		// 如果过滤器是 USER_DECLARED_METHODS，那么就意味着我只处理用户定义的方法，Object 定义的方法不处理
		...
		// 递归。递归的条件就是父类不是 Object，或者父类是 Object，但是并不是特殊的 USER_DECLARED_METHODS 过滤器（这个过滤器不处理 Object）
		if (clazz.getSuperclass() != null && (mf != USER_DECLARED_METHODS || clazz.getSuperclass() != Object.class)) {
			doWithMethods(clazz.getSuperclass(), mc, mf);
		} else if (clazz.isInterface()) {
			// 如果 class 是接口，那么获取继承的接口，递归
	...
	/**
	 * 这个方法会获取实现的接口的 default 方法。如果一个类的继承中，多个类都实现了这个接口，返回的 Method 将会重复出现这个接口定义的 default 方法。
	 */
