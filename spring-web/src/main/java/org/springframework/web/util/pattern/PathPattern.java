...
public class PathPattern implements Comparable<PathPattern> {
	 ...
	 * <p>这个方法就是检查 path 是否存在一些 pattern 语法，或者直接是一个精确 direct path
	 *
	 * @since 5.2
	 */
	public boolean hasPatternSyntax() {
		// score 用来衡量模式的权重或复杂度，比如捕获变量 {name} 就是 1， 通配符 * 就是 100
		// catchAll 是一个布尔值，当模式以 ** 或者 {*variable} 结尾时，这个字段时 true
		// ? 在这里是一个通配符，可以匹配任意单个字符
