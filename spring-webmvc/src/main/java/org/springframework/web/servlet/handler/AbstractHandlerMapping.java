...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		// getHandlerInternal 是子类实现的方法
		// 主要就是 3 条路
		// 1. 根据 path 以及各种信息匹配 handlerMethod
		// 2. 根据 path 匹配
		// 3. RouteFunction
		...
		// 确保把 lookupPath 缓存起来，给后面的拦截器或者别的东西用
		// 也有某些 HandlerMapping 在 getHandlerInternal() 方法中就执行了 initLookupPath，缓存好了，那就不用反复执行
		//
		if (!ServletRequestPathUtils.hasCachedPath(request)) {
			initLookupPath(request);
		}

		// 将 handler 包装成 HandlerExecutionChain
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		// 如果 handler 的类型直接是 HandlerExecutionChain，那么运气很好，否则就包装成 HandlerExecutionChain
		HandlerExecutionChain chain = (handler instanceof HandlerExecutionChain
				? (HandlerExecutionChain) handler
				: new HandlerExecutionChain(handler));

		// 遍历拦截器添加到 chain 里面，毕竟我们上面只是创建了一个空的，要加一些拦截器进去
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			if (interceptor instanceof MappedInterceptor) {
